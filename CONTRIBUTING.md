...
感谢你有兴趣?**ai-verify-mcp** 做贡献！无论是修?bug、改进文档、还是添加新功能，都非常欢迎?
...
1. 先搜?[Issues](https://github.com/validpilot/ai-verify-mcp/issues) 确认是否已存?
2. 创建?Issue，标题简明扼?
3. 包含?
...
同样?[Issues](https://github.com/validpilot/ai-verify-mcp/issues) 中提交，标签选择 `enhancement`。说明使用场景和期望效果即可?

### 提交 Pull Request

1. **先开 Issue** ?任何?trivial 的改动，先开 Issue 讨论，避免白?
2. **Fork 仓库** ?点击 GitHub 右上?Fork
...
```bash
node bin/validpilot.js health    # 确认不影响现有功?
node bin/validpilot.js --version # 确认能正常输?
```
...
1. ?`tools/` 下创?JSON 定义文件
2. ?`server.js` 中注册工具名称和处理函数
...
- 使用 `'use strict'`
- 函数使用 `async/await`
- 变量使用 `const`/`let`（不?`var`?
- 错误使用 `try/catch`，错误消息清?
