...
### Q: ValidPilot Verify 是什么？

A: 一个基于 MCP 协议的 AI 编程验证平台，提供 78 个浏览器自动化和验证工具，帮助你验证 AI 生成的代码是否真的能运行。
...
A: 基础功能**完全免费开源**（MIT License），包括浏览器操作、基础验证、错误聚合等。AI 深度分析（如根因分析、修复建议）需要付费，通过云端 API 实现（¥9/月起）。详见[定价说明](https://validpilot.com/pricing)。
...
```bash
npx @validpilot/ai-verify-mcp health
```
...
A: Playwright 是浏览器自动化引擎，ValidPilot Verify 是基于 Playwright 构建的验证工具集，封装了 78 个高阶工具，专门用于 AI 编程验证场景。

### Q: 和 Cypress / Selenium 比怎么样？

A: 定位不同。Cypress/Selenium 是给人写测试用的，ValidPilot Verify 是给 AI 用的验证工具，通过 MCP 协议让 AI 调用。
