...
# 更换端口启动
npx @validpilot/ai-verify-mcp start --http --port 3457
```
...
2. 检查配置文件 JSON 格式
3. 检查命令路径是否正确
4. 手动运行命令测试：`npx @validpilot/ai-verify-mcp health`
...
**原因**：Trae 单个 Server 工具上限 40 个，本项目有 78 个工具。
...
npx @validpilot/ai-verify-mcp validate --url http://localhost:5173
```
...
导出 Trace 后用 `npx playwright show-trace trace.zip` 查看完整的操作回放。
