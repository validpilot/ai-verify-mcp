...
async function postActionErrorCheck(target, actionName, selector) {
  try {
    ...
    const hasNewErrors = newConsoleErrors.length > 0 || newPageErrors.length > 0 || newNetworkErrors.length > 0;
    const totalNewErrors = newConsoleErrors.length + newPageErrors.length + newNetworkErrors.length;
    ...
    if (hasNewErrors) {
      log('WARN', `操作 "${actionName}(${selector})" 后检测到 ${totalNewErrors} 个新错误`, {
        console: newConsoleErrors.length,
    ...
    return {
      ...
      console: newConsoleErrors.map(e => ({ type: e.type, text: (e.text || '').slice(0, 200) })),
...
function getUnifiedErrors(args = {}) {
  ...
  const total = consoleErrors.length + pageErrorRecords.length + networkErrors.length + mcpErrors.length + imageErrorRecords.length;
  const byLevel = {
    error: consoleErrors.filter(e => e.type === 'error').length + pageErrorRecords.length + networkErrors.filter(e => e.status >= 500 || e.failed).length + mcpErrors.length + imageErrorRecords.length,
  ...
  return redact({
    ...
    summary: {
      ...
      severity: {
        ...
        high: networkErrors.filter(e => e.status >= 500 || e.failed).length,
      ...
      // 页面功能状态评估
      pageStatus: pageErrorRecords.length > 0 ? 'blocked' : (networkErrors.filter(e => e.status >= 500).length > 0 ? 'degraded' : 'functional'),
      // 最后操作的错误摘要
...
function instrumentationScript() {
  return `(() => {
    ...
    window.addEventListener('error', event => push({ type: 'window_error', message: event.message, source: event.filename, line: event.lineno, column: event.colno }));
    ...
    if (originalFetch) {
      window.fetch = async function(input, init = {}) {
        ...
        push({ type: 'fetch_start', requestUrl, method, requestBody: short(init.body) });
        try {
          const response = await originalFetch.apply(this, arguments);
          const duration = Math.round(performance.now() - startedAt);
          response.clone().text().then(body => push({ type: 'fetch_end', requestUrl, method, status: response.status, ok: response.ok, duration, responseBody: short(body) })).catch(() => push({ type: 'fetch_end', requestUrl, method, status: response.status, ok: response.ok, duration }));
          return response;
        } catch (error) {
          push({ type: 'fetch_error', requestUrl, method, error: String(error), duration: Math.round(performance.now() - startedAt) });
          throw error;
        }
      };
    }
    ...
    if (OriginalXHR) {
      window.XMLHttpRequest = function() {
        ...
        let startedAt = 0;
        const open = xhr.open;
        xhr.open = function(m, url) { method = m; requestUrl = url; return open.apply(xhr, arguments); };
        const send = xhr.send;
        xhr.send = function(body) {
          startedAt = performance.now();
          push({ type: 'xhr_start', requestUrl, method, requestBody: short(body) });
          xhr.addEventListener('loadend', () => push({ type: 'xhr_end', requestUrl, method, status: xhr.status, duration: Math.round(performance.now() - startedAt), responseBody: short(xhr.responseText) }));
    ...
    history.pushState = function() { const result = pushState.apply(this, arguments); push({ type: 'route', action: 'pushState', to: location.href }); return result; };
    history.replaceState = function() { const result = replaceState.apply(this, arguments); push({ type: 'route', action: 'replaceState', to: location.href }); return result; };
    window.addEventListener('hashchange', () => push({ type: 'route', action: 'hashchange', to: location.href }));
    push({ type: 'instrumented' });
  })();`;
}
...
async function callTool(name, args = {}) {
  ...
  try {
    ...
    case 'browser_errors':
      return text(JSON.stringify(getUnifiedErrors(args), null, 2));
    ...
    case 'auto_fix_pipeline': {
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        const allPatterns = [
          ...
          { name: 'cors_cross_origin', match: /CORS|cross-origin|跨域|Access-Control|被CORS策略阻止/i, severity: 'critical' },
